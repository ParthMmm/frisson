			...
			<div class="flex items-start justify-between">
				...
				<div class="flex items-center gap-3">
					<button
						...
						lastFmState.username
							? 'h-9 gap-1.5 rounded-xl bg-accent-subtle px-2.5 text-accent before:absolute before:-inset-2 before:content-[\'\']'
							: `${iconHit} size-9 ${
									lastFmConnected
										? 'bg-accent-subtle text-accent'
										: 'border border-divider text-ink-secondary hover:bg-canvas'
								}`}"
						onclick={openLastFm}
					>
						<svg
